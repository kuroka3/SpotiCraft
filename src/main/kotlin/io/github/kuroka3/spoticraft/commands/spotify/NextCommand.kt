...
class NextCommand : CommandChildren {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (sender !is Player) { return false }

        when (SpotifyManager.request(SpotifyManager.Requests.NEXT, sender.uniqueId)) {
            SpotifyManager.Responses.OK -> sender.sendMessage("Skipped to next")
